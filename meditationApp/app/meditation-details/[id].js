...
const MeditationDetails = () => {
  ...
  const displayTabContent = () => {
    if (activeTab === "About") {
      return (
        <About
          title={meditationItem?.title || "Meditation"}
          info={meditationItem?.description ?? "No background data provided for this meditation."}
        />
      );
      ...
      return (
        <View style={styles.specificsContainer}>
          ...
          <View style={styles.pointsContainer}>
            {(meditationItem?.instructions ?? ["Sit comfortably.", "Close your eyes.", "Focus on your breath."]).map((item, index) => (
  ...
  const onShare = async () => {
    try {
      const result = await Share.share({
        message: `Check out this meditation: ${meditationItem?.title || "Meditation"} (${meditationItem?.duration || "10 mins"})`,
      });
